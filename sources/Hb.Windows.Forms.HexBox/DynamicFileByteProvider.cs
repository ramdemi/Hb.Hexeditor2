...
namespace Hb.Windows.Forms
{
    ...
    public sealed class DynamicFileByteProvider : IByteProvider, IDisposable
    {
		...
		public void SaveAsChanges(string outfile)
		{
			...
			long dataOffset = 0;
			if (!HasChanges()) // if not modified write file stream to out file
			{
				_stream.CopyTo(t);
				t.Close();
			}
			else
			{
				for (DataBlock block = _dataMap.FirstBlock; block != null; block = block.NextBlock)
				{
					...
					MemoryDataBlock memoryBlock = block as MemoryDataBlock;
					if (memoryBlock == null)//read from file
					{
					...
					{
						int memoryOffset = (int)memoryBlock.Length;
						t.Position = dataOffset;
						t.Write(memoryBlock.Data, 0,  memoryOffset);
					}
					dataOffset += block.Length;
				}
				t.Close();
			...
			if (!_readOnly)
			{
				_stream = File.Open(outfile, FileMode.Open, FileAccess.ReadWrite, FileShare.Read);
			}
			else
			{
				_stream = File.Open(outfile, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
			}
			ReInitialize();
		}
